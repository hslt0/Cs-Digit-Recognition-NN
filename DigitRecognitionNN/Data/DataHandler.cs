...
public static class DataLoader
{
    public static List<DataPoint> LoadMnist(string filename, int fallbackCount = 1000)
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (!int.TryParse(parts[0], out var label))
                continue;

            var pixelsRaw = parts.Skip(1).Select(byte.Parse).Select(b => (float)b).ToArray();
            var pixels = NormalizePixels(pixelsRaw);
    ...
    {
        Shuffle(data);
        var trainCount = (int)(data.Count * trainRatio);
